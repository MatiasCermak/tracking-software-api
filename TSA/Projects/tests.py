...
class ProjectTest(TestCase):
    ...
    def test_filter_projects(self):
        ...
        self.assertEqual(cnt, 2)
        filters = dict(owner=self.user1.id)
        response = self.browser.post(reverse('filter_projects-list'), filters)
        self.assertEqual(response.status_code, 200)
        cnt = len(response.data)
        self.assertEqual(cnt, 1)
        filters = dict(client=self.client2.id)
        response = self.browser.post(reverse('filter_projects-list'), filters)
        self.assertEqual(response.status_code, 200)
        cnt = len(response.data)
        self.assertEqual(cnt, 1)
    ...
    def test_filter_tickets(self):
        ...
        response = self.browser.post(reverse('filter_tickets-list'), {'state': Ticket.NEW})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 1)
        response = self.browser.post(reverse('filter_tickets-list'), {'area': User.MAINTENANCE})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 1)
        response = self.browser.post(reverse('filter_tickets-list'), {})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 2)
        response = self.browser.post('/api/auth/login/', {'email': 'ux@ux.com', 'password': 'ux123456'})
        rj = json.loads(response.content)
        self.browser.defaults['HTTP_AUTHORIZATION'] = 'Bearer {}'.format(rj.get('access'))
        response = self.browser.post(reverse('filter_tickets-list'), {'area': User.PROJECT_MANAGEMENT})
        self.assertEqual(response.status_code, 403)
        response = self.browser.post(reverse('filter_tickets-list'), {'project': self.project1.id})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 1)
        response = self.browser.post(reverse('filter_tickets-list'), {'state': Ticket.NEW})
        self.assertEqual(response.status_code, 204)
        response = self.browser.post(reverse('filter_tickets-list'), {'state': Ticket.CLOSED})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 1)
        response = self.browser.post(reverse('filter_tickets-list'), {})
        self.assertEqual(response.status_code, 200)
